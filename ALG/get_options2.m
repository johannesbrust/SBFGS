...
%
% 
function par = get_options2()
% convergence
   	par.MaxIter 		= 200;             	% max number of iterations (50)
...
%    par.alg       		= 'bfgs';
      par.alg       		= 's-bfgs';    
   	par.addUnknown      = 'setRatio';      % way to add unknown part: addQuadObj, addNonLinObj, addNonLinObj_Proj(setRatio)
	par.exactRatio		= 0.90;				% ratio to add known part, valid if alg = 'setRatio'. (0.5)
	...
	par.checkInertia	= 2;				% check inertia or not: 0: not check (1)check 2: check descent direction
