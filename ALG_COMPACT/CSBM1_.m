%%%%%%% CSBM1 (compact-structured-BFGS Minus Version 1) %%%%%%%%%%%%%%%%%%
%
...
% 04/09/19, J.B., Debugging version (computes limited memory matrices
% explicitly)

function [bool_conv,sol] = CSBM1_(prob,par,func)
...
S_           = zeros(n,m);
Q_           = zeros(n,m); % inv(Psi0)Y
R_           = zeros(m,m);
D_           = zeros(m,1);
YQ_          = zeros(m,m); % Y'*Q = Y'*inv(Psi0)*Y 

% Matrices to compute with B
Y_           = zeros(n,m);
Qtil_        = zeros(n,m); % Psi0*S
L_           = zeros(m,m);
SQtil_       = zeros(m,m); % S'*Qtil = S'*Psi0*S
...
scalB0      = 10;
trialNum    = 0;
while ~all(eig(CSB)>0) % Possibly use of eigs(CSB,1) ?
    
    trialNum    = trialNum+1;
    CSB         = prob.Hes + power(scalB0,trialNum)*eye(n);
    %       all(scalB0>0);
...
while (nIter < maxIter) && (doMainLoop == 1)
    ...
    st                      = linsolve(Psi0,-g,optsLSS) + S(:,midx(1:cidx))*buff(1:cidx) -...
    ...
    if (par.stopBFGS==1)
        ...
        if (par.skipBFGS==1)
            ...
            if((nIter+1) <= m)
                ...
                Q_(:,1:cidx-1) = Q_(:,2:cidx);
                Y_(:,1:cidx-1) = Y_(:,2:cidx);
                Qtil_(:,1:cidx-1) = Qtil_(:,2:cidx);
                ...
                R_(1:cidx-1,1:cidx-1)    = R_(2:cidx,2:cidx);
                D_(1:cidx-1)             = D_(2:cidx);
                YQ_(1:cidx-1,1:cidx-1)   = YQ_(2:cidx,2:cidx);
                
                L_(1:cidx-1,1:cidx-1)    = L_(2:cidx,2:cidx);
                SQtil_(1:cidx-1,1:cidx-1)= SQtil_(2:cidx,2:cidx);
            ...
            R_(1:cidx,cidx)      = S_(:,1:cidx)'*bk; %buff(1:cidx);
            D_(cidx,1)           = R_(cidx,cidx);
            
            YQ_(cidx,1:cidx)     = bk'*Q_(:,1:cidx);
            YQ_(1:cidx,cidx)     = YQ_(cidx,1:cidx)';
            
            L_(cidx,1:cidx-1)    = sk'*Y_(:,1:cidx-1);
            
            SQtil_(cidx,1:cidx)  = sk'*Qtil_(:,1:cidx);
            SQtil_(1:cidx,cidx)  = SQtil_(cidx,1:cidx)';
            
            %Mat_new = Mat - (ak*ak')/alpha + bk*bk'/gamma;
            
            % Errors
            e1 = norm(S(:,midx(1:cidx))-S_(:,1:cidx),'fro');
            e2 = norm(Q(:,midx(1:cidx))-Q_(:,1:cidx),'fro');
            e3 = norm(Y(:,midx(1:cidx))-Y_(:,1:cidx),'fro');
            e4 = norm(Qtil(:,midx(1:cidx))-Qtil_(:,1:cidx),'fro');
            
            e5 = norm(R(:,:)-R_(:,:),'fro');
            e6 = norm(D(:)-D_(:),'fro');
            e7 = norm(YQ(:,:)-YQ_(:,:),'fro');
            e8 = norm(L(:,:)-L_(:,:),'fro');
            e9 = norm(SQtil(:,:)-SQtil_(:,:),'fro');
            
            e10 = norm(S_(:,1:cidx)'*Y_(:,1:cidx) - L_(1:cidx,1:cidx) - R_(1:cidx,1:cidx),'fro');
            e11 = norm(S(:,midx(1:cidx))'*Y(:,midx(1:cidx)) - L(1:cidx,1:cidx) - R(1:cidx,1:cidx),'fro');
