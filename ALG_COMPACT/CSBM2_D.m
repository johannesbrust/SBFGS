%%%%%%% CSBM2_D (compact-structured-BFGS Minus Version 2 Dense) %%%%%%%%%%
%
...
% Version 2 (V2) sets 
%
% Psi0  = delta_k.I + K0, where delta_k = yk'yk / sk'yk.
%
...
% 05/02/19, J.B., Dense version

function [bool_conv,sol] = CSBM2_D(prob,par,func)
...
while (nIter < maxIter) && (doMainLoop == 1)
    ...
    %% Step computation
    % Computation of st = -H*g, where H = inv(B).
...
%     end
    ...
    Psi0                    = Psiinit + delta.*In;
    
    p1(1:cidx)              = -(S(:,midx(1:cidx))'*g);
    
    Psig                    = Psi0 \ (-g); % Dense solve
    ...
    PsiYbuff                = Psi0 \ (Y(:,midx(1:cidx))*(-buff(cidx+1:2*cidx))); % Sparse solve
    
    buff(1:cidx)            = linsolve(R(1:cidx,1:cidx),...
                                (D(1:cidx).*buff(cidx+1:2*cidx)-...
                                    Y(:,midx(1:cidx))'*PsiYbuff-p2(1:cidx)),optsLSUTT);
