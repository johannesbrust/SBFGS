%%%%%%% CSBP1D (compact-structured-BFGS Plus Version 1 Dense) %%%%%%%%%%%%%
%
...
% Version 1 (V1) sets 
%
% A0 = gamma.I = Constant. (I.e., A0 is constant multiple of identity
% matrix)
%
% In this version, the compact structured BFGS matrices coincide with the recursive
% matrices, when k <= m. Except for the quasi-Newton update this version
% uses the same functions as the 'solvenlp' function.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Initial version: J.B., 05/06/19

function [bool_conv,sol] = CSBP1_D(prob,par,func)
...
%     
%     [stepsS,probS,parS] = compute_step(nIter,prob_newS,par,nIter);
%     [stepsC,probC,parC] = compute_step(nIter,prob_newC,par,nIter);
    
    %[steps,prob,par] = compute_step_CSBP(nIter,prob,par,nIter);
    
%     es = norm(steps.x-stepsS.x);
%     ec = norm(steps.x-stepsC.x);
%     esc= norm(stepsS.x-stepsC.x);
%     
%     escsp = norm(stepsCS.x-steps.x);
...
%     [ret, newIter,newStep,prob_new,num_LS,use_zoom,par] = line_search_strongwolfe(iter,steps,prob,par,func);
    ...
    if (par.stopBFGS==1)
        ...
        if (par.skipBFGS==1)
            ...
            % 05/09/19, J.B. Try reinitialize after every m iterations
%             if mod((nIter+1),m) == 0
%                 
%                 gammfac     = abs(gamma);            
%                 epsgam      = 1.0e+11;            
...
%                     L(1:cidx,1:cidx)',-diag(D(1:cidx,1))])\[Qtil(:,midx(1:cidx)),Y(:,midx(1:cidx))]');
                ...
                %05/08/19, J.B., testing a scaled version
                
                %ds = sqrt(diag(SS(1:cidx,1:cidx)));
                
%                 ds = sqrt(diag(Q(:,midx(1:cidx))'*Q(:,midx(1:cidx))));
%                 dy = sqrt(diag(Y(:,midx(1:cidx))'*Y(:,midx(1:cidx))));
%                 
%                 zm = zeros(cidx,cidx);
%                 Ds = [diag(ds),zm;zm,diag(dy)];
%                 
%                 Vs = [Q(:,midx(1:cidx)),Y(:,midx(1:cidx))] / Ds;
%                 Ms = (Ds\[Lv(1:cidx,1:cidx)+Lv(1:cidx,1:cidx)' + diag(Dv(1:cidx)) + gaminit.*SS(1:cidx,1:cidx),L(1:cidx,1:cidx);...
%                      L(1:cidx,1:cidx)',-diag(D(1:cidx,1))])/Ds;
%                  
%                 Mu = [Lv(1:cidx,1:cidx)+Lv(1:cidx,1:cidx)' + diag(Dv(1:cidx)) + gaminit.*SS(1:cidx,1:cidx),L(1:cidx,1:cidx);...
%                      L(1:cidx,1:cidx)',-diag(D(1:cidx,1))];
%                  
%                  % prob_new.Hes +
%                 Mat_new_s =  B0 - Vs*(Ms\Vs');
                % prob_new.Hes + 
                Mat_new = B0 - [Q(:,midx(1:cidx)),Y(:,midx(1:cidx))] * ...
                     (([Lv(1:cidx,1:cidx)+Lv(1:cidx,1:cidx)' + diag(Dv(1:cidx)) + gaminit.*SS(1:cidx,1:cidx),L(1:cidx,1:cidx);...
                     L(1:cidx,1:cidx)',-diag(D(1:cidx,1))])\[Q(:,midx(1:cidx)),Y(:,midx(1:cidx))]');
...
%             prob_newC    = prob_new;
%             
%             prob_newS.Bk = Mat_new_s;
%             prob_newC.Bk = Mat_new_c;
%             prob_new.BkC = Mat_new_c;
...
%     err1    = norm(Mat_new_s-Mat,'fro');
%     err2    = norm(Mat_new_s-prob_new.BkC,'fro');
%     
%     cs      = cond(Ms);
%     cu      = cond(Mu);
%     
