%%%%%%% CSBP3D (compact-structured-BFGS Plus Version 3 Dense) %%%%%%%%%%%%%
%
...
% Version 3 (V3) sets 
%
% K_{k+1} + A0 = gamma_k.I := Non-constant. (I.e., K_{k+1} + A0 is non-constant 
% multiple of identity matrix). The formulas used in this case:
%
% H = inv(B) = 1/gamma_k( I + 1/gamma_k.(Psi_h inv(M_h) Psi_h' )),
%
% where
%
% Q1k   = K_{k+1}Sk
% Q2k   = Vk - K_{k+1}Sk = Vk - Q1k
% Psi_h = [Q2k + gamma_k.Sk, Yk]
% M_h   = - | Dkv + Rkvh + Rkvh' + 1/gamma_k.(Q2k'Q2k) - Sk'Q1k ,    M12  |
%           |   M12'                                             ,    M22 |,
%
% M21   = Rk + 1/gamma.(Yk'Q2k)
% M22   = Dk + 1/gamma.(Yk'Yk)
% Sk'Vk = Lkv + Rkvh + Dkv = Lkv + Rkv
%
% This version requires the computation of Q1k, Q2k and matrices that
% depend on these at each iteration. Thus it is intended for small m.
%
...
function [bool_conv,sol] = CSBP3_D(prob,par,func)
...
%                     L(1:cidx,1:cidx)',-diag(D(1:cidx,1))])\[Qtil(:,midx(1:cidx)),Y(:,midx(1:cidx))]');
                ...
                Mat_new = prob_new.Hes + B0 - [Q(:,midx(1:cidx)),Y(:,midx(1:cidx))] * ...
