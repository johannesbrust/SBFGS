      function [x,f,g,alp_trial,info,nfev,stx,sty,csbm1_out] ...
       = cvsrch_CSBM1(fcn,n,x,f,g,p,alp_trial,c1,c2,xtol, ...
                 stpmin,stpmax,maxfev,prob,par,func)
%   Translation of minpack subroutine cvsrch
%   Dianne O'Leary   July 1991
%
%   cvsrch_CSBM1 is a modification that returns the structure csbm1_out 
%   (5 fields):
%   csbm1_out.bk    = ybar + Kk*sk
%   csbm1_out.sk    = sk
%   csbm1_out.yk    = u(x_{k1})' - u(xk)
%   csbm1_out.Hes   = k''(x_{k1})
%   csbm1_out.gamma = bk'sk
%   X (Take out) csbm1_out.ng = # gradient evaluations
...
%     **********
      ...
      %% Initializations for modified method
      csbm1_out.bk      = [];
      csbm1_out.sk      = [];
      csbm1_out.yk      = [];
      csbm1_out.Hes     = [];
...
%
         ...
         %% J.B., computation might be more efficient with vector Hessian 
         % product sk'Hes
         x = wa + alp_trial * p;
         [f,g, gradu,Hkplus] = feval(fcn,x,prob,par,func);
         nfev = nfev + 1;
         
         csbm1_out.nhes = csbm1_out.nhes + 1;
         csbm1_out.hes  = Hkplus;
...
%
         ...
         if strcmp(par.alg,'s-bfgs')==1 && par.sbfgsAlg==1 && strcmp(par.addUnknown,'addNonLinObj_Proj')==0 && ((info==5) || (info ==1 ))
            ...
            %% J.B., 04/08/19 storing data
            csbm1_out.bk    = bk;
            csbm1_out.sk    = sk;
            csbm1_out.yk    = yk;
            csbm1_out.gamma = gamma;
