...
while ischar(tline)
    ...
    if ~strcmp(tline(1),'%')  && ischar(tline)
        ...
        [ex(p,s),numf(p,s),numg(p,s),numit(p,s),tcpu(s,:,p),tract(p,s)]=...
            runAlgorithm(@LMTR_EIG_inf_2_DENSE_G10_F_C,x0,parsC{sc},numRuns);
        ...
        % Multiple of identiy for unconstrained minimizer
        % c=1
        s = s+1; sc=1;
        [ex(p,s),numf(p,s),numg(p,s),numit(p,s),tcpu(s,:,p),tract(p,s)]=...
            runAlgorithm(@LMTR_EIG_inf_2_DENSE_G1_C,x0,parsC{sc},numRuns);
