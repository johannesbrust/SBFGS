%{
    ...
    Using unconstrained minimizer from dense initial matrix with
...
while ischar(tline)
    ...
    if ~strcmp(tline(1),'%')  && ischar(tline)
        ...
        % Dense initial matrix for unconstrained minimizer
        % First six functions for c=1,2,4,8,10,100
        s=1; 
        [ex(p,s),numf(p,s),numg(p,s),numit(p,s),tcpu(s,:,p),tract(p,s)]=...
            runAlgorithm(@LMTR_EIG_inf_2_DENSE_G10_F_C,x0,parsC{s},numRuns);
        ...
        s = s+1;
        [ex(p,s),numf(p,s),numg(p,s),numit(p,s),tcpu(s,:,p),tract(p,s)]=...
            runAlgorithm(@LMTR_EIG_inf_2_DENSE_G10_F_C_L,x0,parsC{s},numRuns);
...
save('ResultsEXPERIMENT_I_ii_10','ex','numit','t_aver','numf','numg','params');
