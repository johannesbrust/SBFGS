function [x,f,outinfo] = LMTR_EIG_inf_2_DENSE_G10_F_C_L(fun,x0,params)
% LMTR_EIG_inf_2_F - limited memory trust-region algorithm EIG(inf,2) with
% formula for computing the unconstrained minimizer with a dense initial
% matrix.
% % 06/28/17, [LMTR_...]_C_L: J.B addition of an alternative scaling parameter (flexible).
%
% For details, see: 
...
J.B 09/05/17
This method uses an explicit formula to compute the unconstrained minimizer
with a dense initial matrix. The inverse compact representation is given
as:

    H_k = 1/gamma_perp.I + V (M^-1 *(1/gamma - 1/gamma_perp)(V'V)^{-1})V',

here we use:

    gamma_perp_c = max_i gamma_i,  gamma = y'y/s'y.
...
numidec = 0; % number of insufficient function decreases.
...
if (ng<max(1,norm(x0))*gtol)
    ...
    f=f0;
    tcpu=toc(t0);
    outinfo=LMTR_out(numf,numg,tcpu,ex,it,tract,trrej,params,numrst,numskip,numidec);
    return;
end
