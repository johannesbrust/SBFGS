function [x,f,outinfo] = LMTR_EIG_inf_2_DENSE_G1_SVD1_C_L(fun,x0,params)
% LMTR_EIG_inf_2_SVD1_C_L - limited memory trust-region algorithm EIG(inf,2)
% using a dense initial matrix, and an SVD based approach for the
% eigendecomposition (see below).
% 07/17/17, [LMTR_...]_C_L: J.B addition of an alternative scaling parameter (flexible).
% 
% For details, see: 
...
J.B 13/03/17
Use an SVD based approach in the eigendecomposition of B. This is the
implementation of the first of two such approaches, proposed in a
dissertation by X. Lu:

    V = G Sig U', (Sig U' M U Sig) = Q D Q', such that

    V M V' = G Q D (G Q)', with G = V Sig^{-1} U.

J.B 07/16/17

    gamma_perp = lambda * (c*gamma_perp_c) + (1-lambda) * gamma, 

    for lambda in (0, 1) and gamma = || y ||^2 / s'y. 
...
% Checks for alternative scaling parameter inputs
if isfield(params,'c') && ~isempty(params.c)
    c = params.c;
else
    c = 1;
end;
if isfield(params,'lamc') && ~isempty(params.lamc)
    lamc = params.lamc;
else
    lamc = 1;
end;
...
opts3.SYM = true; % For solving (V'V)^{-1}
...
Q = zeros(m2,m2); % orthogonal matrix from the SVD, (SigU'MUSig) = QDQ'
Sig = zeros(m2,m2); % diagonal, eigenvalues V'V = USig^2U'
Sigr= zeros(m2,1); % vector, sqrt(Sig)
...
p1=zeros(m2,1); % For formula calculation
...
%                     else
                        ...
                        Vs=(-alpha/ns)*(Vg_old(maskVg)...
                           +VV(maskVV,maskVV(lindepV))*p(lindepV));
...
%                 else
                    ...
                    Vs=(-alpha/ns)*(Vg_old(maskVg)+VV(maskVg,lindepV)*p(lindepV));
               % end                
    ...
    %% Quasi-Newton step
    % Compute the L2 norm of the quasi-Newton step based on the inverse Hessian
    % representation by Byrd, Nocedal and Schnabel, 1994 and the proposed formula
    % when a dense initial matrix is used:
    % H = 1/gamma_p.I+ V [ M + (1/gamma - 1/gamma_p).(V'V)^{-1} ] V'
    
    % s=-1/delta*(g+V*p), where p=M*V'*g (OLD),
    % s=-alpha*(g+V*p) (NEW)
...
%     p(1:numsy2)=[linsolve(T(1:numsy,1:numsy),p0(1:numsy),opts2);-TiVg(1:numsy)];    
    ...
    if nst > trrad
        ...
        [p1(1:numsy2) Cr]    = linsolve(VV([1:numsy,m+1:m+numsy],[1:numsy,m+1:m+numsy]), Vg(1:numsy2),opts3);
        
        if Cr > 1e-13
            
            p(1:numsy2)             = ((idelta/alpha)*p(1:numsy2)+((idelta-alpha)/alpha)*p1(1:numsy2));
            nst                     =alpha*sqrt(ng^2+2*(p(1:numsy2)'*Vg(1:numsy2))+...
                        p(1:numsy2)'*(VV([1:numsy,m+1:m+numsy],[1:numsy,m+1:m+numsy])*p(1:numsy2)));
...
%             alpha = 1/delta;
    ...
    if ~Nflag
        ...
        % Compute eigendecomposition of VV(pdl,pdl)=Q*Sig^{2}*Q'
        % and R=sqrt(Sig)*Q' in the SVD decomposition of V = GSigQ'.
        [Q(1:numsy2,1:numsy2), Sig(1:numsy2,1:numsy2)]=eig(VV([1:numsy,m+1:m+numsy],[1:numsy,m+1:m+numsy]));  
        Sigr(1:numsy2,1)=sqrt(diag(Sig(1:numsy2,1:numsy2)));
        
        % Use only safely linearly independent columns of V to
        % compute the trial step
        masksvd=find(Sigr>ranktol); 
        rankV=length(masksvd);  % column rank of V
        %lindepV=masksvd;  % index of safely linearly independent columns of V
        lindepV=1:numsy2;
        R(1:rankV,1:numsy2)=diag(Sigr(masksvd))*Q(1:numsy2,masksvd)';          
        ...
        [U(1:rankV,1:rankV),D(1:rankV,1:rankV)]=eig(R(1:rankV,1:numsy2)*...
            (M(1:numsy2,1:numsy2)\(R(1:rankV,1:numsy2)')));        
        lam(1:rankV) = delta-diag(D(1:rankV,1:rankV));
        
        % Compute vectors for solving TR subproblem in the new variables 
        %gpar(1:rankV)=U(1:rankV,1:rankV)'*((Q(masksvd,masksvd)'*Vg(masksvd))./Sigr(masksvd));
        %gpar(1:rankV)=U(1:rankV,1:rankV)'*(R(1:rankV,lindepV)*Vg(lindepV)); % Q(1:rankV,masksvd)
        %gpar(1:rankV)=U(1:rankV,1:rankV)'*(p2(masksvd)./Sigr(masksvd)); % Q(1:rankV,masksvd)
        
        gpar(1:rankV)=U(1:rankV,1:rankV)'*((Q(lindepV,masksvd)'*Vg(lindepV))./Sigr(masksvd));
...
%             else % All other methods computed by original formula
                ...
                %p(1:rankV) = Q(masksvd,masksvd)*((U(1:rankV,1:rankV)*(-vpar(1:rankV)/alpha-gpar(1:rankV)))./Sigr(masksvd));
                
                p(lindepV) = Q(lindepV,masksvd)*((U(1:rankV,1:rankV)*(-vpar(1:rankV)/alpha-gpar(1:rankV))./Sigr(masksvd)));
                
                st=-alpha*(g+V(:,maskV(lindepV))*(p(lindepV)./nV(maskV(lindepV))));
