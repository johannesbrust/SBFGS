function [x,f,outinfo] = LMTR_EIG_inf_2_DENSE_G1_SVD2_C_L(fun,x0,params)
...
J.B 15/03/17
Implementation of the second SVD based decomposition:

    V = G Sig Q', V'V = Q Sig^2 Q', U Lam U' = Sig Q' M^{-1} Q Sig, where
    
    P_{par} = G U = V M^{-1} Q Sig U Lam^{-1}
...
Q = zeros(m2,m2); % orthogonal matrix from the SVD, eig(V'V) = Q Sig^2 Q'
P2 = zeros(m2,m2); % buffer storing M^{-1} Sigr Q' 
Sig = zeros(m2,m2); % diagonal, eigenvalues V'V = Q Sig^2 Q'
Sigr= zeros(m2,m2); % vector, sqrt(Sig)
%Im2 = eye(m2); % identity
...
%                     else
                        ...
                        Vs=(-alpha/ns)*(Vg_old(maskVg)...
                            +VV(maskVV,maskVV(lindepV))*p(lindepV)); % p(1:rankV)
...
%                 else
                    Vs=(-alpha/ns)*(Vg_old(maskVg)+VV(maskVg,lindepV)*p(lindepV)); % p(1:rankV)
               % end                
...
%             alpha = 1/delta;
    ...
    if ~Nflag
        ...
        P2(lindepV,1:rankV) = linsolve(M(1:numsy2,1:numsy2),R(1:rankV,1:numsy2)',opts3);
        
        
        % Compute eigenvalue decomposition of R*inv(M)*R'       
        [U(1:rankV,1:rankV),D(1:rankV,1:rankV)]=eig(R(1:rankV,1:numsy2)*P2(1:numsy2,1:rankV));
                    
        dD(1:rankV,1) = diag(D(1:rankV,1:rankV));
        
        lam(1:rankV) = delta-dD(1:rankV);
        
        
        % Compute vectors for solving TR subproblem in the new variables 
        gpar(1:rankV,1)=((U(1:rankV,1:rankV)'*((P2(lindepV,1:rankV)'*Vg(lindepV))))./dD(1:rankV)); % Q(1:rankV,masksvd)
...
%             else % All other methods computed by original formula
                ...
                % ((U(1:rankV,1:rankV)'*(R(1:rankV,masksvd)*(Mi(masksvd,masksvd)*Vg(lindepV))))'./dD(1:rankV));
                p(lindepV) = P2(lindepV,1:rankV)*(U(1:rankV,1:rankV)*((-vpar(1:rankV)/alpha-gpar(1:rankV))./dD(1:rankV)));
                %p(1:rankV) = ((U(1:rankV,1:rankV)'*(R(1:rankV,masksvd)*(Mi(masksvd,masksvd)*(-vpar(1:rankV)/alpha-gpar(1:rankV)))))'./dD(1:rankV));
