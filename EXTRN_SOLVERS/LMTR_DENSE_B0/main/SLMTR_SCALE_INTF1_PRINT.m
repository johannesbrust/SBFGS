function [x,f,outinfo,DTRo,DTRi] = SLMTR_SCALE_INTF1_PRINT(fun,grad,hprod,x0,params)
% SLMTR_SCALE_INTF1_PRINT (Structured-limited-memory trust-region method with
%       scaling)
% Interface 1, implements a separate 'structured' gradient function.
% PRINT, stores outer and trust-region information:
%   DTRo: [ it,  ; TRs, deltas ]    ((maxit+2)x2)
%   DTRi: [ it_in, delta_in, TRs_in ]      ((maxit+1)x(75+2)
%
...
% Containers for print version
% 10/25/19, J.B.
DTRo    = zeros(maxit+2,2);
maxind  = 75;
DTRi    = zeros(maxit+1,(maxind+2));
...
% 10/25/19, J.B., Store TR-radius (k=0)
DTRo(it+1,1) = trrad;
...
%             if gp < 0; gp = delta; end;
        ...
        % 10/25/19, J.B. Data store
        DTRi((it+1),trit) = trrad; 
    ...
    % 10/25/19, J.B., store Data
    DTRo((it+2),1) = trrad_old;
    DTRo((it+2),2) = delta;
