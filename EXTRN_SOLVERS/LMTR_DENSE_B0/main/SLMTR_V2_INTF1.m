function [x,f,outinfo] = SLMTR_V2_INTF1(fun,grad,hprod,x0,params)
%- SLMTR_V2_INTF1 (Structured-limited-memory trust-region method, Version 2) 
% Interface 1 ---%
%
% Trust-region algorithm of modified structured BFGS formulas from Petra, Chiang,
...
%
% Psi0  = delta_k.I, where delta_k = yk'yk / sk'yk.
%
% This version (Version 2) modifies the computation of yk. For parameter
% \theta \in [0,1]. For given threshold thetaT if norm(s) < thetaT, then
% \theta = norm(s) and
% 
% yk = (g_{k+1} - gk) + \theta.( K_{k+1}sk + (u'_{k+1} - u'_k) - (g_{k+1} -
% gk) ).
% This version only does the structured updates if ||s|| < threshold.
%
% For \theta = 0, this approach reduces the full L-BFGS formula,
% while for \theta = 1, this approach reduces to the full S-L-BFGS formula.
%
...
%
% INPUTS:
% fun: Structured objective (3 calling patterns)
%       [f]         = fun(x);
%       [f,gk,gu]   = fun(x);
%       [gk,gu]     = fun(x);
...
%           .theta  = Version 1 uses this parameter to compute yk.
%
% OUTPUTS:
...
% 09/12/19, J.B., Version 1, implementing theta parameter.
% 09/13/19, J.B., printing ns, nrestart and theta.
% 10/17.19, J.B., Interface update.
...
if isfield(params,'theta') && ~isempty(params.theta)
    theta = params.theta;
else
    theta = 0;
end;
if isfield(params,'thetaT') && ~isempty(params.thetaT)
    thetaT = params.thetaT;
else
    thetaT = 0;
end;
...
if dflag==1
    ...
    fprintf('it\t obj\t\t norm(df)\t norm(dfu)\t norm(dx)\t trrad \t\t nskip \t theta\n');
    fprintf('%d\t %.3e\t %.3e\t %.3e\t ---\t\t --- \t --- \t --- \n',0,f0,ng,norm(gu0));
...
if ngi>gtol  % norm of gradient is too large    ng>gtol*max(1,norm(x))
    ...
    % y = g-g_old;
    % Compute Hessian product only if desired. Take structured step, when
    % ||s|| small.
    if thetaT < ns
        y= (g-g_old);
    else
        theta = ns;
        y= (1-theta).*(g-g_old) + theta.*(hprod(x,s) + (gu-gu_old));
    end
...
%        hasppar = 0;
        ...
        % Branching for scaling parameter. If structured BFGS is used
        % dense B0 is ommited.
%         if theta == 0
...
%             it,f,ng,nst_inf_2,trrad_old);
        ...
        fprintf('%d\t %.3e\t %.3e\t %.3e\t %.3e\t %.3e \t %d \t %.3e\n',...
            it,f,ng,norm(gu),ns,trrad_old,numskip,theta);
    end    
