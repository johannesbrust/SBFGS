function EXPERIMENTS_II_A_FUNC(saveFiles,m,probPath,dataPath,nsol,selectp)
% EXPERIMENTS_II_A_FUNC: Experiments II make comparisons with full-memory
% S-BFGS with varying memory. Uses S-BFGS-M
%
...
%-------------------------------------------------------------------------%
% 11/20/19, J.B.
% 11/21/19, J.B., Modification to selectively take out full-memory solvers.
...
for j = 1:nsel
    ...
    for k = 1:nsol
        ...
        if k <= 4
            ...
            par.Scaling   = k; 
            outData{j,k}  = EX_CSBMSV3_F_MF_SCALE(prob,func,par);
...
if saveFiles == 1
    ...
    dname = [dataPath,'experiments','_II_A','_m',num2str(m)];
